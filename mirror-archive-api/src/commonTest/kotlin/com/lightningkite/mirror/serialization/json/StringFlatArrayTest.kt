...
class StringFlatArrayTest {
    ...
    fun <T> test(value: T, type: MirrorType<T>): T {
        val result = StringFlatArrayFormat.toArray(type, value)
        val schema = StringFlatArrayFormat.columns(type)
        schema.indices
                .asSequence()
    ...
    fun listString() {
        ...
        val map = StringFlatArrayFormat.toArray(IntMirror.list, listOf(1, 2, 3, 4))
        println(map[0])
        val back = StringFlatArrayFormat.fromArray(IntMirror.list, map)
        println(back)
    }
    ...
    fun reflectiveWithSkip() {
        ...
        val decoder = StringFlatArrayDecoder(StringFlatArrayFormat.context, StringFlatArrayFormat.stringFormat, resultWithNulls){false}
        val back = decoder.decodeSerializableValue(type)
    ...
    fun reflectivePartialShallow() {
        ...
        val encoded = StringFlatArrayFormat.toArrayPartial(ZooMirror, default, 333, path)
    ...
    fun reflectivePartialMultivalue() {
        ...
        val encoded = StringFlatArrayFormat.toArrayPartial(ZooMirror, default, IntData(333), path)
