...
class SecureDatabase<T : Any, USER>(
        ...
        val fieldRules: Map<FieldInfo<T, *>, FieldRule<T, *, USER>>
) {
    data class FieldRule<T : Any, V, USER>(
            val fieldInfo: FieldInfo<T, V>,
        ...
        return object : Database<T> {
            ...
            fun Sort<T>.safety(): Sort<T>? {
                return when (this) {
                    ...
                    is Sort.Field<*, *> -> if (fieldRules[this.field as FieldInfo<T, *>]?.queryable?.invoke(user) != false) this else null
            ...
            fun Condition<T>.safety(): Condition<T> {
                return when (this) {
                    ...
                    is Condition.Field<*, *> -> {
                        if (fieldRules[this.field as FieldInfo<T, *>]?.queryable?.invoke(user) != false) this
            ...
            fun Operation<T>.safety(): Operation<T> {
                return when (this) {
                    ...
                    is Operation.Field<*, *> -> fieldRules[this.field as FieldInfo<T, *>]?.operation?.let { opMod ->
                        val untypedOpMod = opMod as (USER, Operation<Any?>) -> Operation<Any?>
                        Operation.Field(field as FieldInfo<T, Any?>, untypedOpMod.invoke(user, operation as Operation<Any?>))
                    } ?: this
                    else -> this
                }
            }
