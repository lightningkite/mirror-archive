...
class CborCopy(val updateMode: UpdateMode = UpdateMode.BANNED, val encodeDefaults: Boolean = true) : AbstractSerialFormat(), BinaryFormat {
    // Writes output entry as plain [key, value] pair, without bounds.
    inner class CborEntryWriter(encoder: CborEncoder) : CborWriter(encoder) {
    ...
    inner class CborMapWriter(encoder: CborEncoder) : CborListWriter(encoder) {
    ...
    open inner class CborListWriter(encoder: CborEncoder) : CborWriter(encoder) {
        override fun writeBeginToken() = encoder.startArray()

        override fun encodeElement(desc: SerialDescriptor, index: Int): Boolean = true
    }

    // Writes class as output [fieldName, fieldValue]
    open inner class CborWriter(val encoder: CborEncoder) : ElementValueEncoder() {

        init {
            context = this@CborCopy.context
        }
        ...
        //todo: Write size of output or array if known
    ...
    class CborEncoder(val output: OutputStream) {
        ...
        fun composeNumber(value: Long): ByteArray =
                if (value >= 0) composePositive(value) else composeNegative(value)

        fun composePositive(value: Long): ByteArray = when (value) {
        ...
        fun composeNegative(value: Long): ByteArray {
    ...
    inner class CborEntryReader(decoder: CborDecoder) : CborReader(decoder) {
        var ind = 0
    ...
    inner class CborMapReader(decoder: CborDecoder) : CborListReader(decoder) {
        override fun skipBeginToken() = decoder.startMap()
    }

    open inner class CborListReader(decoder: CborDecoder) : CborReader(decoder) {
        var ind = -1
        var size = -1
    ...
    open inner class CborReader(val decoder: CborDecoder) : ElementValueDecoder() {
    ...
    companion object : BinaryFormat {
        ...
        const val FALSE = 0xf4
        const val TRUE = 0xf5
        const val NULL = 0xf6

        const val NEXT_FLOAT = 0xfa
        const val NEXT_DOUBLE = 0xfb

        const val BEGIN_ARRAY = 0x9f
        const val BEGIN_MAP = 0xbf
        const val BREAK = 0xff

        const val HEADER_STRING: Byte = 0b011_00000
        const val HEADER_NEGATIVE: Byte = 0b001_00000
        const val HEADER_ARRAY: Int = 0b100_00000
